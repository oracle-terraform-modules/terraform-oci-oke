...
Or if you are passing the key contents directly (as Terraform Cloud would), enter the following as "Sensitive" variables in a Terraform Cloud Workspace:

ssh_private_key 
ssh_public_key 

(Use the contents of the actual files)
