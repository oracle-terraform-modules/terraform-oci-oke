...
If your private ssh key has a different name or path than the default "~/.ssh/id_rsa" e.g "~/.ssh/dev_rsa", you will need to add the private key to your ssh agent:
...
* vcn_name: this is the name of the vcn that will be prepended to the label prefix
