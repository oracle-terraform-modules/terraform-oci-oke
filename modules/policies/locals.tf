...
locals {
  ...
  policy_statement = (var.use_encryption == true) ? "Allow dynamic-group ${oci_identity_dynamic_group.oke-kms-cluster[0].name} to use keys in compartment id ${var.compartment_id} where target.key.id = '${var.key_id}'" : ""
}
