...
|`cluster_image_policy_enabled`
|Whether to enforce use of signed images from registry.
|true/false
|false
...
|`existing_key_id`
|id of existing KMS key. *Required* if _use_encryption_ is set to *true* or _cluster_image_policy_enabled_ is set to *true*
|
|
|===
