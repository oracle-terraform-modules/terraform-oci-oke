...
resource "oci_identity_policy" "operator_instance_principal_dynamic_group" {
  ...
  name           = "${var.label_prefix}-operator-instance-principal-dynamic-group"
  statements     = ["Allow dynamic-group ${var.dynamic_group} to use dynamic-groups in tenancy"]
  count          = (var.oke_kms.use_encryption == true && var.operator.bastion_enabled == true && var.operator.operator_instance_principal == true && var.reuse == false) ? 1 : 0
}
...
resource "oci_identity_policy" "oke-kms" {
  ...
  count          = (var.oke_kms.use_encryption == true && var.reuse == false) ? 1 : 0
}