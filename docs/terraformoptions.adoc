...
|Whether to enable instance_principal on the operator server. Refer to {uri-docs}/instructions.adoc/#enabling-instance_principal-on-the-operator-host[instance_principal]
|true/false
|true
...
|check_node_active
|Setting this variable to true will make the script to wait till the OKE node become active
|true/false
|false
